...
const SearchBar = () => {
  ...
  const onSearch = (event: React.FormEvent) => {
    ...
    if (searchQuery === "") {
      // If the search query is empty, redirect to the classes page
      router.push('/classes');
    } else {
      // If the search query is not empty, proceed with the search
      const encodedSearchQuery = encodeURIComponent(searchQuery);
      router.push(`/classes/search?q=${encodedSearchQuery}`);
    }
